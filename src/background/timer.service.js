...
export async function testCompletePomodoro() {
  try {
    ...
    if (
      ...
      timerState.currentSession !== 'pomodoro'
    ) {
...
// Dev/Test helper: Fast-complete the current running session (any type) and credit full planned duration
export async function testCompleteSession(sessionFromRequest) {
  try {
    const timerState = await getTimerState();
    if (!timerState || !timerState.isRunning) {
      return { success: false, error: 'No session is currently running' };
    }

    const session = sessionFromRequest || timerState.currentSession;

    // Credit analytics with full planned duration for the current session
    try {
      const planned = timerState.totalTime || 25 * 60;
      await analyticsService.logSessionComplete({
        session,
    ...
    // Determine next session and counts like normal completion
    if (session === 'pomodoro') {
      const newPomodoroCount = (timerState.pomodoroCount || 0) + 1;
      const newSessionCount = (timerState.sessionCount || 0) + 1;
      const isLong = newPomodoroCount % 4 === 0;
      const nextSession = isLong ? 'long-break' : 'short-break';
      const nextDuration = timerState.sessions[nextSession].duration;
      updates = {
        ...updates,
        pomodoroCount: newPomodoroCount,
        sessionCount: newSessionCount,
      ...
      updates = {
        ...updates,
        sessionCount: (timerState.sessionCount || 0) + 1,
        currentSession: 'pomodoro',
        timeLeft: duration,
        totalTime: duration,
      };
    } else if (session === 'custom') {
      // For custom, just stop the timer and keep current session with 0 left
      updates = {
        ...updates,
    ...
    console.error('Error in testCompleteSession:', error);
    return { success: false, error: error.message };
  }
}
