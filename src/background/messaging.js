  ...
  try {
    switch (request.action) {
      ...
      case ACTIONS.ADD_CATEGORY_SITE:
        await categoriesService.addCategorySite(
          request.url,
          request.categoryId
        );
      ...
      case ACTIONS.REMOVE_CATEGORY_SITE:
        await categoriesService.removeCategorySite(
          request.url,
          request.categoryId
        );
        sendResponse({ success: true });
      ...
      case ACTIONS.TOGGLE_CATEGORY_BLOCKING:
        await categoriesService.toggleCategoryBlocking(
          request.categoryId,
          request.enabled
        );
        sendResponse({ success: true });
      ...
      case ACTIONS.SAVE_CATEGORY_METADATA:
        await categoriesService.saveCategoryMetadata(
          request.categoryId,
          request.metadata
        );
        sendResponse({ success: true });
      ...
      case ACTIONS.GET_ACTIVE_CATEGORY_SITES:
        const activeCategorySites =
          await categoriesService.getActiveCategorySites();
      ...
      case ACTIONS.ANALYTICS_GET_HISTORY:
        const history = await analyticsService.getSessionHistory({
      ...
      case ACTIONS.SET_SETTINGS:
        ...
        if (
          oldSettings.unifiedModeEnabled !== request.settings.unifiedModeEnabled
        ) {
          await unifiedOrchestrator.handleModeSwitch(
            request.settings.unifiedModeEnabled
          );
        }
        sendResponse({ success: true });
        break;
